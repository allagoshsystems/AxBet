...
function setConnection(online,label){const el=$("connection");if(!el)return;el.textContent=label||(online?"ESPNcricinfo Live":"Feed Offline");el.className="connection "+(online?"online":"offline")}
function visible(){
...
async function loadMatches(){
  ...
  try{
    const ts=Date.now();
    const urls=[`/api/matches?ts=${ts}`,`/matches.json?ts=${ts}`];
    let r=null,lastError=null;
    for(const url of urls){
      try{
        const controller=new AbortController();
        const timer=setTimeout(()=>controller.abort(),9000);
        const candidate=await fetch(url,{cache:"no-store",signal:controller.signal,headers:{"Cache-Control":"no-cache","Pragma":"no-cache"}});
        clearTimeout(timer);
        if(candidate.ok){r=candidate;break;}
        lastError=new Error(`${url}: ${candidate.status}`);
      }catch(err){lastError=err;}
    }
    if(!r)throw lastError||new Error("match data unavailable");
    const x=await r.json();
    const events=Array.isArray(x.events)?x.events:(Array.isArray(x.matches)?x.matches:[]);
    if(!x.success && !events.length)throw new Error(x.error||"ESPNcricinfo feed unavailable");

    data={
      series:Array.isArray(x.series)?x.series:[],
      events:events.map(e=>({...e,status:String(e.status||"").toUpperCase()}))
    };
    localStorage.setItem("axbet_last_match_snapshot",JSON.stringify({savedAt:Date.now(),...data}));
    setConnection(true,x.stale?"ESPNcricinfo (stale)":"ESPNcricinfo Live");
    renderSeries();
    renderEvents();
  }catch(e){
    const cached=localStorage.getItem("axbet_last_match_snapshot");
    if(cached){
      try{
        const snapshot=JSON.parse(cached);
        data={series:Array.isArray(snapshot.series)?snapshot.series:[],events:Array.isArray(snapshot.events)?snapshot.events:[]};
        renderSeries();
        renderEvents();
      }catch(_){}
    }else{
...
window.addEventListener("online",()=>loadMatches());window.addEventListener("offline",()=>setConnection(false));
