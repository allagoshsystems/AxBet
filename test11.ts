...
async function test(url) {
  ...
  cards.each((i, el) => {
      ...
      if (statusText.length > 60 || 
          normStatus.includes(titleText.toLowerCase().substring(0, 5) + titleText.toLowerCase().substring(0, 5)) || 
          (team1 && statusText.replace(/\s+/g,'').includes(team1.replace(/\s+/g,'') + team1.replace(/\s+/g,''))) || 
          (team2 && statusText.replace(/\s+/g,'').includes(team2.replace(/\s+/g,'') + team2.replace(/\s+/g,''))) ||
          statusText.includes("Match details not yet available")) {
          statusText = "UPCOMING";
      }
      
      if(titleText.includes("England") || titleText.includes("Greece")){
         console.log(`- ${titleText} | Final Status: ${statusText}`);
      }
  });
}
